<?php 
	...
	if (!function_exists('send_email')) {
		 ...
		 function send_email($recipient_email, $recipient_name, $subject, $body, $reply_to=false)
		{
	        ...
	        $mail->Port = 25; 
	         ...
	         $mail->setFrom($row->app_email,$row->app_name);
	        $mail->addReplyTo($row->app_email,$row->app_name);
	...
	if(!function_exists('get_seller_option')){
		function get_seller_option($key, $seller_settings_row)
		{
			$str = '';
			$ci = &get_instance();
			if(!empty($seller_settings_row)){
				foreach($seller_settings_row as $item){
					if($item->option_key == $key){
