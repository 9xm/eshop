...
class Api_model extends CI_Model
{
    ...
    public function products_filter($type, $id='', $limit='', $start=1, $brands='',$min='', $max='',$order_by='',$size='', $keyword='', $user_id='',$category=''){
    ...
    public function get_wishlist($user_id, $limit='', $start=1){
    ...
    public function get_compare($user_id, $limit='', $start=''){

        $where = array('user_id' => $user_id);
        
        $this->db->select('compare.*');
        $this->db->select('product.id AS product_id,product.product_title, product.category_id, product.sub_category_id, product.brand_id, product.offer_id, product.product_slug, product.product_desc, product.featured_image, product.product_mrp, product.selling_price, product.you_save_amt, product.you_save_per, product.total_views, product.color, product.delivery_charge,product.max_unit_buy,product.`product_size`, product.`rate_avg`, product.`total_rate`, product.status');
        $this->db->from('tbl_compare compare');
        $this->db->join('tbl_product product','compare.product_id = product.id','LEFT');
        $this->db->where($where);
        if($limit!='' && $limit!=0){
          $this->db->limit($limit, $start);
        }
        $this->db->order_by('compare.id', 'DESC');
        $query = $this->db->get();
        return $query->result();
    }
    ...
    public function get_vseen_orders($limit=0){
    ...
    public function get_refund_data($order_id=0, $groupBy='')
    {
      ...
      $seller_id=$this->session->userdata('id');
        //$where = array('refund.gateway !=' => 'cod', 'order_items.vid'=>$seller_id);
        $where = array('order_items.vid'=>$seller_id);
    ...
    public function todays_orders()
    {
      ...
      $where=array("order.order_status <> "=> "-1", "DATE_FORMAT(FROM_UNIXTIME(order.order_date), '%d-%m-%Y') =" => date('d-m-Y'), 'tbl_order_items.vid'=>$user_id);
      ...
      $this->db->join('tbl_order_items','tbl_order_items.order_id= order.id','LEFT');  
    ...
    public function vendor_commission($limit='', $start='', $keyword='')
    {
        $this->db->select('*');
        $this->db->from('tbl_vendor_commission');
        if($limit!=''){
          $this->db->limit($limit, $start);
        }
        if($keyword!=''){
          $this->db->like('product_title',stripslashes($keyword));
          $this->db->or_like('order_id',stripslashes($keyword));
          $this->db->or_like('vendor_name',stripslashes($keyword));
          $this->db->or_like('vendor_email',stripslashes($keyword));
        }
        $this->db->order_by('id', 'DESC');
        return $this->db->get()->result();
    }

    public function vendor_commission2($limit='', $start='', $keyword='')
    {
        $this->db->select('*');
        $this->db->from('tbl_vendor_commission');
