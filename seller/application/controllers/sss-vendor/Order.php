...
class Order extends CI_Controller {
    ...
    function index()
    {
        ...
        $this->db->select('order.*,tbl_order_details.order_unique_id,tbl_order_details.order_date,tbl_order_details.order_date,tbl_order_details.order_status');
        ...
        if($this->input->get('ord_status')){
            ...
            //$data['order_list'] = $this->common_model->selectWhere('tbl_order_details',array('order_status' => $ord_status,'vid'=>$this->session->userdata('id')),'DESC');
            $where=array('tbl_order_details.order_status' => $ord_status,'order.vid'=>$this->session->userdata('id'));

            if($this->input->get('date_filter')){

                $date_filter=trim($this->input->get('date_filter'));

                //$data['order_list'] = $this->common_model->selectWhere('tbl_order_details',array("DATE_FORMAT(FROM_UNIXTIME(`order_date`), '%d-%m-%Y') =" => $date_filter, 'order_status' => $ord_status,'vid'=>$this->session->userdata('id')),'DESC');
                $where=array("DATE_FORMAT(FROM_UNIXTIME(`tbl_order_details.order_date`), '%d-%m-%Y') =" => $date_filter, 'order_status' => $ord_status,'order.vid'=>$this->session->userdata('id'));
            }
        }
            ...
            //$data['order_list'] = $this->common_model->selectWhere('tbl_order_details',array("DATE_FORMAT(FROM_UNIXTIME(`order_date`), '%d-%m-%Y') =" => $date_filter,'vid'=>$this->session->userdata('id')),'DESC');
            $where=array("DATE_FORMAT(FROM_UNIXTIME(`tbl_order_details.order_date`), '%d-%m-%Y') =" => $date_filter,'order.vid'=>$this->session->userdata('id'));
        }
        else
        {
            $where=array('order.vid'=>$this->session->userdata('id'));
        }
        if ($where!='') 
        {
           $this->db->where($where);
        ...
        $this->template->load('vendor/template', 'vendor/page/orders', $data); // :blush:
    } 
    ...
    public function order_summary()
    {
        ...
        if(!empty($this->Order_model->get_order($order_no))){
            ...
            $this->template->load('vendor/template', 'vendor/page/view_order', $data); // :blush:
        }
        else{
    ...
    public function print_order()
    {
        ...
        $data['order_data'] = $this->Order_model->get_order($order_no);
        $this->load->view('vendor/page/print_order', $data); // :blush:
    }
    ...
    {  
        ...
        $this->load->view('vendor/page/order_status_update', $data); // :blush:
    }
    ...
    {
        $row=$this->api_model->get_vseen_orders(3);
        $html='';

        foreach ($row as $key => $value) {

            $old_title='New order is placed by '.$this->common_model->selectByidParam($value->user_id, 'tbl_users', 'user_name');
            ...
            $html.='<li><a title="'.$old_title.'" href="'.site_url("vendor/orders/".$this->common_model->selectByidParam($value->order_id, 'tbl_order_details', 'order_unique_id')).'"><div class="message"><div class="content"><div class="title">'.$title.'</div><div class="description"><span>ORDER ID:</span> '.$this->common_model->selectByidParam($value->order_id, 'tbl_order_details', 'order_unique_id').'</div><div class="description"><span>Date:</span> '.date('d M, Y', $this->common_model->selectByidParam($value->order_id, 'tbl_order_details', 'order_date')).'</div></div></div></a></li>';
        }
    ...
    public function ordervseen()
    {
        $data = array(
            'vseen'=>'1',
        );
        $this->db->where('vseen','0');
        $this->db->where('vid',$this->session->userdata('id'));
        $inserted = $this->db->update('tbl_order_items',$data);
        return;
    }
}