...
class Order extends CI_Controller {
    ...
    public function update_product_status() 
    {
        ...
        if($this->input->post('product_id')==0){
            ...
            foreach ($data_arr as $key => $value) 
            {
                ...
                if($status=='4'){
                    $this->seller_commission($value->order_id, $value->product_id);
    ...
    public function seller_commission($order_id, $product_id)
    {
       
                        
        $this->db->select('order_items.id as order_main_id, order_items.vid as vendor_id, order_items.order_id as order_id, order_items.product_price, order_items.total_price as total_price, order_items.delivery_charge, subcat.seller_percentage as seller_percentage, vendor.user_name as vendor_name, vendor.user_email as vendor_email, vendor.bankacno as vendor_account, vendor.ifsc as  vendor_ifsc, order_items.product_title as  product_title');
        $this->db->from('tbl_order_items order_items');
        $this->db->join('tbl_product product','product.id = order_items.product_id','LEFT');
        $this->db->join('tbl_sub_category subcat','subcat.id = product.sub_category_id','LEFT');
        $this->db->join('tbl_partner vendor','vendor.id = order_items.vid','LEFT');
        $this->db->where(array('order_items.pro_order_status' => 4, 'order_items.order_id'=> $order_id, 'order_items.product_id'=> $product_id ));
        ...
        if(!empty($resultSet)){
            foreach ($resultSet as $key => $value){
                ...
                if(!empty($value->seller_percentage)){
                    $product_mrp = floatval($value->total_price) - floatval($value->total_price) * floatval($value->seller_percentage) / 100;
                    //$product_mrp = floatval($value->total_price) * floatval($value->seller_percentage) / 100;
                    $product_percentage = floatval($value->seller_percentage);
                }
                ...
                $data = array(
                    ...
                    'vendor_account'=>$value->vendor_account,
                    'vendor_ifsc'=>$value->vendor_ifsc,
                    'product_title'=>$value->product_title,
                    'order_id'=>$value->order_id,
